...
function StatusBadge({ status }: { status: "active" | "at-risk" | "completed" }) {
  ...
  if (status === "at-risk") return <Badge tone="warning">At risk</Badge>;
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button leftIcon={<Download size={16} />}>Export Report</Button>}
      />
        ...
        <Card title="Cohort Performance" subtitle="Completion trends by group launch cycle.">
          <div className="grid gap-4">
            {[71, 64, 83].map((value, index) => (
              <div key={index}>
                <p className="mb-1 text-sm">Cohort {index + 1}</p>
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button leftIcon={<Plus size={16} />}>Invite Worker</Button>}
      />
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        <label className="relative min-w-[260px] flex-1">
          ...
          <input
            ...
            placeholder="Search worker, department, or group"
      ...
      {groups.data ? (
        <div className="grid gap-4 md:grid-cols-2">
          {groups.data.map((group) => (
            <Card key={group.id} title={group.name} subtitle={`${group.course} · Teacher: ${group.teacher}`}>
              <div className="grid grid-cols-2 gap-3 text-sm">
                <p className="adapted-muted">Seats: {group.seats}</p>
                <p className="adapted-muted">Active: {group.activeStudents}</p>
              </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card title="Risk Radar" subtitle="Groups requiring intervention">
          <ul className="grid gap-3 text-sm">
            <li className="rounded-xl bg-amber-50 p-3 text-amber-800">Hospitality B1 - Morning: 4 students inactive for 7+ days.</li>
            <li className="rounded-xl bg-blue-50 p-3 text-blue-800">Service English Intensive: strong weekly growth (+12%).</li>
            <li className="rounded-xl bg-slate-100 p-3 text-slate-700">Recommend: schedule targeted speaking clinic this week.</li>
          </ul>
        </Card>
      </div>
    </div>
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button leftIcon={<Plus size={16} />}>Generate Codes</Button>}
      />
      {codes.data ? (
        <DataTable
          data={codes.data}
